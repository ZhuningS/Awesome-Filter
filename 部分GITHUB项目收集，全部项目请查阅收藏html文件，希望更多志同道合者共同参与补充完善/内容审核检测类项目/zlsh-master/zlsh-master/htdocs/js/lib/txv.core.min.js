/*! TenVideo - v1.0.0 - 2014-07-12 16:14:17
 * Copyright (c) 2014
...
if (typeof Live == "undefined" || !Live) {
	...
	if (typeof QQLive != "undefined" && !QQLive) {
		if (typeof $j === "function") {
			$j.extend(Live, QQLive);
		} else if (typeof Live.object != "undefined") {
	 ...
	 * @type {String}
	 */
	Live.lastmodify = "$Date: 2014-07-08 12:50:28 +0800 (Tue, 08 Jul 2014) $";
};
...
Live.cookie = {
	...
	get : function(name) {
		...
		// return (pos = s.indexOf(' ' + name + '=')) > -1 ? s.slice(pos +=
		// name.length + 2, s.indexOf(';', pos)) : '';
...
$j.extend($j.util, {
	...
	showAtCenter : function(loginwin, w, h, isNeedSetExtendStyle) {
		if (!loginwin)
			return;
		var notIframe = (window == top),
			doc = notIframe ? document : top.document,
			scrhei = doc.body.scrollTop,
			clihei = doc.documentElement.clientHeight,
			$loginwin = $j(loginwin), zIndex;
		w = w || $loginwin.width() || 373;
		h = h || $loginwin.height() || 235;
		isNeedSetExtendStyle = (isNeedSetExtendStyle === false) ? true : false;
		if (0 == scrhei)
			scrhei = doc.documentElement.scrollTop;
		if (0 == clihei)
			clihei = doc.body.clientHeight;
		zIndex = parseInt($loginwin.css("zIndex"));
		$loginwin.css({
					width : w + "px",
					height : h + "px",
					top : (notIframe) ? "50%" : ((clihei / 2 + scrhei) - h / 2 + "px"),
					left : "50%",
					marginLeft : -w / 2 + "px",
					marginTop : (-h / 2 + "px"),
					position : "fixed",
		...
		if (isNeedSetExtendStyle) {
			$loginwin.css({
						padding : '0px',// 某些浏览器下，这个字段的默认值不为0，要重置一下
						backgroundColor : '#ffffff' // 当用户使用护眼屏保时，容器变换大小时会出现绿色，强制将背景色设为白色可以解决问题
					});
		}
		if ($j.util.useHtml5()) {
...
$j.extend({
	...
	getToken : function() {
		var skey = Live.login.getSkey(),
			token = !!skey ? $j.time33(skey) : "";
		return token;
	}
});

// 将$变量重定义到jQuery
Live.noConflict = function() {
...
Live.Login = function(conf) {
	...
	$j.each(['ClearLoginStatus', 'OpenLogin', 'CloseLogin', 'Login', 'LoginCancel', 'Logout', 'UserInfoChanged'], function(key, val) {
		if (!callbacks[val]) {
			var cb = $j.Callbacks('unique');
			callbacks[val] = cb;
			self['add' + val + 'Callback'] = cb.add;
			self['remove' + val + 'Callback'] = cb.remove;
			self['empty' + val + 'Callback'] = cb.empty;
			self['on' + val] = cb.fireWith;
		}
	});
};
$j.each(['getUin', 'getNick', 'getAvatar', 'getSkey', 'openLogin', 'logout', 'isLogin', 'clearLoginCookie', 'getEncUin', 'setStatus'], function(key, val) {
...
Live.template = {
	...
	trimpath: "http://qzs.qq.com/tencentvideo_v1/js/trimpath.min.js?max_age=60480000",
	/**
	 * 当前是否正在拉取JS
	 *
	 * @type {Boolean}
	...
	_getJsDtd: null,
	/**
	 * 模板调用堆栈数，用来判断在模板嵌套使用时，子模板使用父模板的modifier
	 * @type {Number}
	 * @ignore
	 */
	_callStack: 0,
	...
	MODIFIERS: {
		...
		formatTime: function(str) {
			if (!/\d+/.test(str))
				return str;
			var v = parseInt(str) * 1000; // javascript需要的是毫秒 但是unix返回的是秒
			var d = new Date(v);
	 ...
	 *          tplid 模板id
	 * @param {String|Object}
	 *          [containerid] 容器id|容器jQuery对象，如果不传入此参数，则在回调函数中传入渲染后的HTML字符串
	 * @param {Boolean}
	 *          isClear 是否清除容器原始内容，默认是true。如果不清除使用的是append模式，不会销毁再重建原有的dom元素
	 * @param {Object}
	 *          modifiers 模板函数
	 * @param {Number}
	 *          filltype 填充类型 1从页面textarea来 2为静态化模板字符串
	 * @param {function}
	 *          callback 回调函数，传入渲染好的html。callback(html)
	 * @param {Boolean}
	 *          isResetLink 是否重置链接
	 */
	fillHtml: function(json, tplid, containerid, isClear, modifiers, filltype, callback, isResetLink) {
		function fill(_json, _tplid, _containerid, _isClear, _modifiers, _filltype, _callback, _isResetLink) {
			...
			var $container, hasContainer = true,
				myTemplateObj, result = "";
			...
			if ((filltype == Live.template.FILL_TYPE.TPL_ID && !document.getElementById(_tplid)) || (filltype == Live.template.FILL_TYPE.TPL_STR && !_tplid)) {
				return;
			}
			...
			//记录当前模板调用堆栈，如果是顶层模板，暂存modifiers；如果是子模板，使用顶层模板的modifiers
			...
			Live.template._callStack--;
			if (Live.template._callStack == 0) { //顶层模板渲染完清空暂存的modifiers
		...
		if (typeof TrimPath == "undefined") {
			if (Live.template._isLoadingJS == false) {
				...
				Live.template._getJsDtd = $j.getScript(Live.template.trimpath);
			}
			...
			//TrimPath已存在的时候直接返回渲染好的HTML内容，共子模板嵌套调用时使用
...
Live.txv = {
	 ...
	 * 最后一次更新时间
	 */
	lastmodify : "$Date: 2011-09-29 10:37:23 +0800 (Thu, 29 Sep 2011) $",
...
Live.txv.path = {
	 ...
	 * @type String
	 */
	search_cgi : "http://v.qq.com/search.html?pagetype=3",
	...
	/**
	 * 智能联想v2 lastupdated by ancheltong 2013.12.02
	 * 
	 * @type String
	 */
	search_suggest2 : "http://s.video.qq.com/smartbox?plat=2&ver=0&num=10&otype=json&query=",

	/**
	 * 评分CGI地址
	 * 
	 * @type String
	 * @default http://video1.qq.com/v1/pingfencgi?
	 */
	...
	/**
	 * MV收藏到绿钻音乐盒CGI(由音乐同学提供,联系人sevenzhang)
	 * 
	 * @type String
	 */
	...
	/**
	 * MV收藏到音乐盒“喜欢”的查询CGI，用于查询指定的MVID是否被指定的UIN收藏
	 * 
	 * @type String
	 */
	...
	/**
	 * 收藏到空间MV秀(由音乐同学提供,联系人sevenzhang)
	 * 
	 * @type String
	 */
	 ...
	 * @author arikshi
	 *         @example
	 *         t=1&cids=cid1|cid2.  或者t=0&cids=cid.
	 */
	 ...
	 * @type String
	 */
	getVuserInfo : "http://c.v.qq.com/vuserinfoget?otype=json",
	/**
	 * 是否已经进入过个人中心feeds页面
	 * @type String
	 */
	checkfollowviewed : "http://c.v.qq.com/checkfollowviewed?&otype=json",
	/**
	*
	*个性化推荐,cid =15*1,tablist=2001,playright=2,size=5,page_type=,uin=xxx
	*/
	ulike: "http://like.video.qq.com/fcgi-bin/recommand?playright=2"

};
txv.Event = {
	loadedFollow : "loaded_follow"
};
$j.extend(Live.template.MODIFIERS, {
	
	
});
Live.txv.template = Live.template;
/*
 * @include "../../extend/qzfl.jquery.js" @include
 * "../common/jquery.extend.util.js" @include "./txv.define.js" @include
 * "../common/live.login.js" @include "./txv.config.js" @include
 * "./txv.common.js"
 */

$j.extend(Live.Login.prototype, {
	        /**
	        	 * 是否已经登录
	        	 * 
	        ...
	        getUin : function() {
		        ...
		        if (!uin || uin <= 10000) {
			        uin = parseInt(Live.cookie.get(this.getConfig('luinCookie')).replace(/^o0*/g, ""), 10);
			        if (!uin || uin <= 10000) {
				        return 0;
			        }
		        }
	        ...
	        getAvatar : function(size) {
		        ...
		        if (arr[1] && arr[1] == uin) {
			        if (arr[2]) {
				        size = txv.login.getValidAvatarSize(size);
	        ...
	        getValidAvatarSize : function(size) {
		        var sizeArr = txv.login.getConfig('avatarSize');
		        if ($j.inArray(size, sizeArr) == -1) {// 没有这种格式的图片
			        return sizeArr[0];
		        }
		        return size;
	        },
		        ...
		        if (txv.login.getNick() == "" || reloadnick == true) {
			        $j.getScript(txv.login.getConfig('login_cgi') + "&_=" + new Date().valueOf() + "&g_tk=" + $j.getToken(), function() {
				                if (typeof QZOutputJson != "undefined") {
					                if (QZOutputJson.result && QZOutputJson.result.code == "0") { // 获取成功
						                if (typeof QZOutputJson.encuin != "undefined") { // 把加密的uin打cookie到v.qq.com下
						                ...
						                if (location.host.indexOf("qq.com") >= 0) {
							                Live.cookie.set(txv.login.getConfig('nickCookie'), [QZOutputJson.nick, txv.login.getUin(), QZOutputJson.qqface, QZOutputJson.has100].join("|"), location.host, "/");
						                }
						                txv.login.showOnline(QZOutputJson.nick, QZOutputJson.qqface, QZOutputJson.has100);
					                ...
					                else if (QZOutputJson.result && QZOutputJson.result.code != "0") { // 失败
						                if (QZOutputJson.result.code == "100001") { // 验证失败，可能是skey和uin不匹配
							                txv.login.logout();
						                }
						                else if (QZOutputJson.result.code == "2504") { // 登录认证成功,
							                // 但获取昵称失败
	         ...
	         * 是否是弱登录状态
	         * 
	         * @return {Boolean} 是否是弱登录态
	         */
	        isLowLogin : function() {
		        var skey = $j.trim(Live.cookie.get("skey")),
			        uin = $j.trim(Live.cookie.get(txv.login.getConfig("uinCookie")));
		        return txv.login.isLogin() && (!skey || !uin);
	        },
	        /**
	         * 显示当前在线的用户信息
	         * 
	         * @param {Boolean}
	         *          isShowNick 是否显示昵称 如果为flase，显示为QQ号码 主要用于已经登录但是获取昵称失败的情况
	         * @param {string} avatar
	         * @param {bool} exist100
	         */
	        showOnline : function(nickname, avatar, exist100) {
		        var login_nick = $j("#login_nick,[data-type='nickname'],._nickname"),
			        login_action = $j("#login_action"),
			        $login_avatar = $j("[data-type='avatar'],._avatar");
		        login_nick.text(nickname && decodeURIComponent(nickname) || txv.login.getNick());
		        login_action.text(txv.login.getConfig('logout_btn_text'));
		        login_action.unbind("click");
		        login_action.bind("click", txv.login.logout);

		        if (typeof exist100 == "undefined") {// 从cookie里取是否有100
			        var n = Live.cookie.get(this.getConfig('nickCookie')),
				        arr = n.split("|");
			        if (n == "" || arr.length < 4) {
				        exist100 = 1;
		        ...
		        if ($login_avatar.size() > 0) {
			        var img = new Image(),
				        avatar_url = typeof avatar == "string" ? avatar : txv.login.getAvatar();
			        img.onload = function() {
				        $login_avatar.each(function(i, element) {
					                var $el = $j(element),
						                size = txv.login.getValidAvatarSize(parseInt($el.attr("data-avatarsize"), 10)),
						                url = avatar_url;
					                if (exist100 == 1) {
						                url = avatar_url.replace(/([\?&]?s=)40([&$])/, "$1" + size + "$2");
					                }
					                $el.attr("src", url);
				                });
			        }
	        ...
	        showNotLogin : function() {
		        var login_nick = $j("#login_nick,_nickname"),
			        login_action = $j("#login_action");
	        ...
	        showMask : function(centerId, w, h, showAtCenter, onScroll) {
		        ...
		        if (!document.getElementById("mask_layer")) {
			        ...
			        var maskDiv = $j('<div id="mask_layer" class="mask_layer"></div>');
					$j(document.body).append(maskDiv);
		        }
		        else {
			        $j("#mask_layer").show();
		        }
		        ...
		        if (showAtCenter) {
			        txv.login.showAtCenter(document.getElementById(centerId), w, h);
	        ...
	        hideMask : function() {
		        $j("#mask_layer").hide();
		        if ($j.isFunction(txv.login.onHideMask)) {// 此回调仅内部对象使用
	         ...
	         * 
	         * @param {Object}
	         *          _config 配置项 详情点击<a href="txv.login.config.html">此处</a>查看
	         */
	        openLogin : function(_config) {
		        txv.login.isOpend = true;
		        $j.extend(txv.login.config, _config);
		        ...
		        var url = "", loginwin,
			        ...
			        baseUrl = txv.login.getPtloginUrl(),
			        applogopic = "http://i.gtimg.cn/qqlive/images/20130521/i1369106644_1.jpg?max_age=6048000",
			        siteLogo = "";
		        if (document.getElementById("login_win") == null) {
			        
					var ifmDiv = $j('<div id="login_win"><iframe name="_login_frame_quick_" id="_login_frame_quick_" frameborder="no" scrolling="no" width="100%" height="100%" src="about:blank"></iframe></div>');
					$j(document.body).append(ifmDiv);
		        }
		        loginwin = document.getElementById("login_win");
		        if (!loginwin) {
			        setTimeout(function() {
				                txv.login.openLogin(_config);
			                }, 50);
			        return;
		        }
		        ...
		        url = [baseUrl, "&appid=", txv.login.config.appid, "&f_url=loginerroralert&qlogin_auto_login=0", "&s_url=", escape("http://v.qq.com/toolpages/redirect.html?clientjumpurl=" + escape(clientjumpurl || window.location.href) + "&jumpurl=" + jumpurl)].join('');
		        document.getElementById("_login_frame_quick_").src = url;
		        ...
		        // 带着登录态打开登录框 我们统计下数据
		        try {
			        if (txv.login.isLogin() && txv.login.getSkey() != "") {
	        ...
	        /**
	        	 * 根据登陆模式，确定是否要使用弱登录
	        	 * 
	        	 * @return {string} ptlogin的url
	        	 */
	        getPtloginUrl : function() {
		        var url = "http://ui.ptlogin2.qq.com/cgi-bin/login?link_target=blank&target=self";
		        if (!this.getConfig('useStrictLogin')) {
			        url += "&low_login=1";
		        }
		        return url;
	        },
...
//		        if (txv.common.useHtml5()) {// html5的设备不跟着滚动
...
//		        }
	        ...
	        clearLoginCookie : function(isFireCb) {
		        ...
		        if (txv.login.getConfig('useStrictLogin') == false) {// 使用弱登录，清除lskey
			        Live.cookie.set(txv.login.getConfig('luinCookie'), "", ".qq.com", "/", -24);
			        Live.cookie.set("lskey", "", ".qq.com", "/", -24);
		        }
	        ...
	        getSkey : function() {
		        ...
		        if (this.getConfig('useStrictLogin')) {
			        ...
			        return skey || $j.trim(Live.cookie.get("lskey"));
		        }
	        }
        });
...
$j.extend(Live.txv.login, {
	        /**
	        	 * 配置项
	        	 * 
	        	 * @namespace Live.txv.login.config
	        	 * @type {Object}
	        	 */
	        config : {
		        /**
		        ...
		        quickLogin : true,
		        /**
		        	 * 登录成功的跳转url，为空字符串时表示ajax无刷新登录
		        	 * 
		        	 * @type String
		        	 */
		        jumpurl : "",
		        /**
		        	 * ajax登录，登录成功，但是跨域执行ajax失败以后默认跳转页
		        	 * 
		        	 * @type String
		        	 */
		        clientjumpurl : null,
				 ...
				 * 续期skey
				 */
				renew_skey: "http://uple.qq.com/?t=aio_page&op=renew",
		        /**
		        	 * 个性显示
		        	 */
		        show_uniq : false,
	        ...
	        /**
	        	 ...
	        	 *          _config 配置项
	        	 * @param {Boolean}
	        	 *          iscallback 如果登录成功，是否需要执行回调函数
	        	 */
	        init : function(config, iscallback) {
...
txv.login.setConfig(txv.login.config);
...
// 注册全局方法
(function() {
	var onResize = $j.isFunction(window.ptlogin2_onResize) ? window.ptlogin2_onResize : QZFL.emptyFn,
		onClose = $j.isFunction(window.ptlogin2_onClose) ? window.ptlogin2_onClose : QZFL.emptyFn,
		onSuccess = $j.isFunction(window.ptlogin2_onSuccess) ? window.ptlogin2_onSuccess : QZFL.emptyFn,
		rdt = $j.isFunction(window.__rdt__) ? window.__rdt__ : QZFL.emptyFn;
	/**
	 * 统一登录框大小发生变化时的回调函数
	 * 
	 * @param {Number}
	...
	window.ptlogin2_onResize = function(width, height) {
		// Live.console.print("width=" + width + "," + "height=" + height);
		document.getElementById("_login_frame_quick_").style.height = height + "px";
		document.getElementById("_login_frame_quick_").style.width = width + "px";
		txv.login.showAtCenter(document.getElementById("login_win"), width, height);
	 ...
	 * 
	 * @param {Boolean}
	 *          isSuc 关闭后是否执行ptlogin2_onSuccess回调
	 */
	window.ptlogin2_onClose = function(isSuc) {
